...
##############################
    ...
    fol='10'+ str(fol+1)
    folder = 'mode_'+(mod)+ '_net_' + str(nett) +'_host_'+ str(host)+ '_algo_' + str(algo)+ '_protocol_' + str(prot)+'_server_' + str(sertype)

    
    os.system('mkdir -p /home/dash/testbed/Data/Raw/'+fol+'/'+ folder)
    
    os.system('cd /home/dash/testbed/goDASH/godash/config/con && sudo rm -R *')
    ...
    if prot=='tcp':
       test_dict['quic']='off'
       test_dict['url']='https://www.godashbed.org/x264/tearsofsteel/DASH_Files/full/tearsofsteel_enc_x264_dash.mpd'
    else:
       test_dict['quic']='on'
       test_dict['url']='https://www.godashbed.org:4444/x264/tearsofsteel/DASH_Files/full/tearsofsteel_enc_x264_dash.mpd'
...
# pcap capture                
def pcap(mod, host, algo, nett, prot, sertype):
	print("Pcap capturing s1-eth10 ..........\n")
	#os.system('sudo tcpdump -i s1-eth10 -U -w  /home/dash/testbed/Data/Raw/mode_'+(mod)+ '_net_' + str(nett) +'_host_'+ str(host)+ '_algo_' + str(algo)+ '_protocol_' + str(prot)+'_server_' + str(sertype)+'/mode_'+(mod)+ '_net_' + str(nett) +'_host_'+ str(host)+ '_algo_' + str(algo)+ '_protocol_' + str(prot)+'_server_' + str(sertype)+'_ap.pcap')
...
def player(mod, client, host, algo, nett, prot, sertype,fol):
     ...
     client.cmd('cd /home/dash/testbed/goDASH/godash && ./godash -config ./config/configure.json >/home/dash/testbed/Data/Raw/'+str(fol)+'/'+'mode_'+str(mod)+ '_net_' + str(nett)+'_host_'+ str(host)+ '_algo_' + str(algo)+ '_protocol_' + str(prot)+'_server_' + str(sertype)+'/mode_' + str(mod) +'_net_' + str(nett) + '_client_' + str(client)+ '_host_' + str(host)+ '_algo_' + str(algo)+ '_protocol_' + str(prot)+'_server_' + str(sertype)+'.txt && echo ....Streaming done_' + str(client))
...
def ditgr(s):
    print(s.cmd('cd /home/dash/testbed/D-ITG-2.8.1-r1023/bin  && ./ITGRecv'))
...
def ditgs(c,bt):
    ...
    print(c.cmd('cd /home/dash/testbed/D-ITG-2.8.1-r1023/bin && ./ITGSend script_file1'+str(bt)+' -l sender.log -x receiver.log'))
...
if __name__ == '__main__':
    ...
    if a:
       n=Process(target=pcap, args=(mod,host,algo,nett,prot,st,))
       n.start()
       b=True  
